...
**In progress (2026-08-30):** the repository half of the unit is on `feat/launch`. Pre-cutover verification is done and recorded in `docs/launch-checklist.md`: 135 unit and 39 e2e green, all-static build, mobile Lighthouse 97 to 98 across the five visible routes with 100 in the other three categories, and axe clean on those five plus the 404 page in both themes. Best practices reached 100 for the first time, 96 through U4 to U8 having been the missing favicon; `app/icon.svg` is that fix, and `tests/e2e/fixtures.ts` narrowed with it from swallowing every resource 404 to swallowing only the document's own. The content export ran while Heroku is still up: 7 jobs and 1 article pulled, the article identical to the migrated MDX bar U6's two recorded deviations, the jobs matching `content/cv.yaml` except for deliberate CV edits and one company, Leaf Group, that the hand-written CV of record dropped. The raw export is archived outside the repo and the Leaf Group record is quoted in the checklist, so nothing is lost when the app is deleted. Of the two dependency items the unit carried, TypeScript 7 is taken (7.0.2 is stable, everything is clean on it, `tsc --noEmit` drops to 0.4s) and `experimental.turbopackRustReactCompiler` is skipped (still experimental, and it buys build time this project does not need). What is left is everything that needs André's hands: the Vercel settings, the preview verification, the cutover, and the decommission.
