...
- [x] **U9. Launch: verification, Vercel cutover, content export, decommission**
...
**Outcome (2026-08-30):** Shipped. andrevital.com serves `main` at `1c586a1`, and the old backend is gone.

The repository half merged as PR #71: pre-cutover verification recorded in `docs/launch-checklist.md` (140 unit and 40 e2e green, all-static builds both ways, mobile Lighthouse 97 to 98 across the five visible routes, axe clean on those five plus the 404 page in both themes), `app/icon.svg` as the site's first favicon, which is what moved best practices from 96 to 100, and the content export pulled while Heroku was still up. Of the two dependency items the unit carried, TypeScript 7 is taken and `experimental.turbopackRustReactCompiler` is skipped.

The cutover cost one failed build and taught one thing worth keeping: **Vercel's Promote to Production rebuilds rather than re-aliases**, so a green preview can still fail on promote. This one did. A `NODE_ENV=production` project variable left over from 2022 is read by pnpm at install time, which makes it skip devDependencies, so `tsx` was missing and `prebuild`'s `pnpm cv` died with `tsx: command not found` eleven seconds in. Preview builds had never hit it because they restore a warm `node_modules` cache; the Production environment keeps its own cache and installed cold, so the first build ever to run there was the one to break. Deleting the variable fixed it, and Next sets `NODE_ENV=production` for `next build` itself, so the draft filter in `lib/content.ts` is untouched. andrevital.com served the old deployment throughout, Vercel leaving the previous production alias in place when a build errors.

Verified against the live domain rather than the preview: five routes 200 with correct content types, every legacy redirect landing on its target with the same status codes the preview gave, `/work`, `/craft` and `/craft/logo-draw` 404 the same as an unknown route, a sitemap of exactly five URLs canonical to andrevital.com, www 308 to the apex on a valid certificate, and in both Chromium and WebKit a first visit that draws and hands over, a second visit inline, no drawing at all under reduced motion, a theme that survives a reload and no console or page errors. Mobile Lighthouse on production: `/` 98, `/about` 98, `/contact` 100, `/writing` 96, the post 98, all with 100 / 100 / 100 in the other three, so R33 holds on the real domain.

The decommission corrected one recorded fact. S3 held **12 objects, not one**: the earlier count came from the CMS media API, which reports one media entry, while the bucket also kept Strapi's derived sizes and two orphaned re-uploads. Nothing was lost, both 1.7 MB originals being byte-identical to the archived copy, and the only unarchived file, the old 192px PWA icon, was pulled down before the bucket went. The Heroku app and its Postgres are deleted, the bucket is deleted, the CMS access key is deleted and its IAM user stripped of every key, group and policy, the `production` and `chore/google-cloud-migration` branches are gone from origin, the 143 MB of leftover `packages/` is removed, and `git grep` finds no backend reference outside this plan, the checklist and one pointer in `README.md`. Worth noting what that key could do: it belonged to a `dev` user whose only group granted EC2, RDS and S3 full access, so a config var on a hobby CMS carried far more than image uploads.

Definition of Done re-read against the live site: every line holds. R23 included, `career-ops/cv.md` being a symlink to `content/cv.md`, which `scripts/build-cv.tsx` generates from `cv.yaml`. Two things are open but neither is a launch blocker: the `dev` IAM user itself survived its `delete-user`, most likely a console login profile, and "logo variants correct" is met as specified while André's own note on the dark nav says the mark loses readability there, which is its own open item rather than a defect in what shipped.
