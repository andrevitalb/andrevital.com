...
- Accessibility during the full intro: content is `inert` and `aria-hidden`; no focus is moved; after the intro the document is focusable as normal.
...
- `LogoIntro` (client) reads `data-intro` once on mount; in `full` mode renders the veil and the hero mark with `layoutId="logo-mark"`, sets `inert` and `aria-hidden` on `main`, listens for pointer, key and wheel to skip, and on completion unmounts the hero mark so the nav mark (same `layoutId`) receives the layout animation; then removes the veil and releases `inert`. In `inline` mode the nav mark draws in under 1 second; with reduced motion no draw runs.
...
- Budget check: Home JavaScript under 150 KB compressed; `motion` imported from `motion/react-client` where possible.
...
- Full intro sets `inert` on `main` and removes it at completion; a keydown during the intro ends it within one frame and the marker is set.
